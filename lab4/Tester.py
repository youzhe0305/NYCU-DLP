...
class Dataset_Dance(torchData):
    def __init__(self, root, transform, mode='test', video_len=7, partial=1.0):
        ...
        data_num = len(glob(os.path.normpath('./Demo_Test/*')))
        data_num = 5 # modified
        for i in range(data_num):
            self.img_folder.append(sorted(glob(os.path.normpath(os.path.join(root , f'test/test_img/{i}/*'))), key=get_key))
            self.label_folder.append(sorted(glob(os.path.normpath(os.path.join(root , f'test/test_label/{i}/*'))), key=get_key))
        print()
...
class Test_model(VAE_Model):
    ...
    def load_checkpoint(self, args):
        if self.args.ckpt_path != None:
            checkpoint = torch.load(self.args.ckpt_path, map_location=torch.device(args.device))
