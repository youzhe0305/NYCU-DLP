...
def get_key(fp):
    if sys.platform.startswith('win'):
        filename = fp.split('\\')[-1] # for win
    elif sys.platform.startswith('linux'):
        filename = fp.split('/')[-1] # for linux
...
class Dataset_Dance(torchData):
    ...
    def __init__(self, root, transform, mode='train', video_len=7, partial=1.0):
        ...
        if mode == 'train':
            self.img_folder     = sorted(glob(os.path.normpath(os.path.join(root, 'train/train_img/*.png'))), key=get_key)
            self.prefix = 'train'
        elif mode == 'val':
            self.img_folder     = sorted(glob(os.path.normpath(os.path.join(root, 'val/val_img/*.png'))), key=get_key)
            self.prefix = 'val'
        else:
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.video_len):
            if sys.platform.startswith('win'):
                label_list = self.img_folder[(index*self.video_len)+i].split('\\') # for win
            elif sys.platform.startswith('linux'):
                label_list = self.img_folder[(index*self.video_len)+i].split('/') # for linux
            label_list[-2] = self.prefix + '_label'
            
            img_name    = self.img_folder[(index*self.video_len)+i]
            if sys.platform.startswith('win'):
                label_name = '\\'.join(label_list) # for win
            if sys.platform.startswith('linux'):
                label_name = '/'.join(label_list) # for linux
